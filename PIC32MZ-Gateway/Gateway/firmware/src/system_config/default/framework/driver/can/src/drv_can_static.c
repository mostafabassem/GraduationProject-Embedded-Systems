...
static CAN_TX_MSG_BUFFER __attribute__((coherent)) can_message_buffer0[2*2*16];
...
{	
    ...
    /* Assign the buffer area to the CAN module.
     ...
     /* Configure CAN_ID_1 Channel for CAN_RX_FULL_RECEIVE operation. Allocate 5 message buffer, and assign low medium priority for transmissions. */			
     PLIB_CAN_ChannelForReceiveSet(CAN_ID_1, CAN_CHANNEL1, 5, CAN_RX_FULL_RECEIVE);
...
bool DRV_CAN0_Receive(CAN_CHANNEL channelNum, int *address, uint8_t *DLC, uint8_t* message)
{
    int EchoDLC = 0;
...
bool ChannelMessageReceive(CAN_CHANNEL channelNum, int address, uint8_t DLC, uint8_t* message)
{
    int EchoDLC = 0;
    CAN_RX_MSG_BUFFER *RxMessage;     
    RxMessage = (CAN_RX_MSG_BUFFER *)PLIB_CAN_ReceivedMessageGet(CAN_ID_1, channelNum);
